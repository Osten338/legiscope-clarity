      ...
      <div className="grid md:grid-cols-2 md:gap-8 xl:gap-12">
        ...
        <ContainerScroll className="min-h-[400vh] space-y-8 py-12">
          {PROCESS_PHASES.map((phase, index) => (
            <CardSticky key={phase.id} index={index + 2} className="rounded-2xl border p-8 shadow-md backdrop-blur-md">
...
const Work = () => {
  return (
    <div className="container min-h-svh place-content-center bg-white px-6 text-stone-900 xl:px-12">
      <div className="grid md:grid-cols-2 md:gap-8 xl:gap-12">
        <div className="left-0 top-0 md:sticky md:h-svh md:py-12">
          <h5 className="text-xs uppercase tracking-wide">our work</h5>
          <h2 className="mb-6 mt-4 text-4xl font-bold tracking-tight">
            Showcasing our <span className="text-black">latest projects</span> and collaborations
          </h2>
          <p className="max-w-prose text-sm">
            Explore our portfolio of recent projects where we've helped businesses transform their digital presence.
        ...
        <ContainerScroll className="min-h-[300vh] space-y-8 py-12">
          {WORK_PROJECTS.map((project, index) => (
            <CardSticky key={project.id} index={index + 2} className="rounded-2xl border overflow-hidden shadow-md backdrop-blur-md">
              <div className="relative">
                ...
                />
                <div className="p-8">
                  <h2 className="mb-4 text-2xl font-bold tracking-tighter text-black">
                    {project.title}
                  </h2>
...
const Achievements = () => {
  return (
    <div className="container min-h-svh place-content-center bg-stone-50 px-6 text-stone-900 xl:px-12 py-24">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        {ACHIEVEMENTS.map((achievement) => (
          <div 
            key={achievement.id} 
            className="p-8 rounded-2xl flex flex-col items-center justify-center text-center aspect-square"
            style={{ backgroundColor: achievement.bg, color: 'white' }}
          >
