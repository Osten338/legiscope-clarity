...
export const RegulationCard = ({
  ...
  const handleSaveRegulation = async () => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.from('saved_regulations').insert([{
        ...
        <CollapsibleTrigger className="w-full">
          <div className="flex items-start gap-4 text-left">
            ...
            <div className="flex-grow">
              ...
              <p className="text-slate-600 font-serif">{regulation.description}</p>
            </div>
            <Button variant="ghost" size="icon" className="flex-shrink-0 mt-1" onClick={e => {
            e.stopPropagation();
            if (!isRegulationSaved) {
              handleSaveRegulation();
            }
          }} disabled={saving || isRegulationSaved}>
              {isRegulationSaved ? <BookmarkCheck className="h-5 w-5 text-sage-600" /> : <Bookmark className="h-5 w-5 text-sage-600" />}
            </Button>
          </div>
        </CollapsibleTrigger>
        <CollapsibleContent className="mt-4 space-y-4">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {regulation.checklist_items.map(item => <li key={item.id} className="flex items-start gap-3 text-slate-600">
